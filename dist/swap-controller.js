  ...
  if (role === "initiator") {
    ...
    if (cpLock >= 5e8) {
      ...
      if (spvSupported(theirChain)) {
        try {
          tsNow = await spvVerifiedTipTimeSec(client, theirChain, buried.freshHeight);
        } catch {
...
var SwapController = class _SwapController {
  ...
  async rehydrate() {
    ...
    if (await this.deps.durable.get(refundBroadcastKey(rec.id))) {
      ...
      if (r.finalized) {
        this.setResumeGate("refund-finalized");
        return;
      }
      if (await this.recoverUtxoRefundRace()) {
      ...
      if (await this.finalizeOrResubmitEvmRefund()) {
        this.setResumeGate("refund-finalized");
        return;
      }
    ...
    if (await this.deps.durable.get(claimBroadcastKey(rec.id))) {
      ...
      if (c.finalized) {
        this.setResumeGate("claim-finalized");
        return;
      }
   ...
   */
  async rebroadcastClaimIfDropped() {
    const rec = this.record;
    if (!await this.deps.durable.get(claimBroadcastKey(rec.id))) return false;
    if (chainConfigs[this.theirChain]?.isEvm) return false;
    let redeemScript;
    try {
      ({ redeemScript } = this.counterpartyLeg("rebroadcastClaimIfDropped"));
    } catch {
      return false;
    }
    const priorRaw = await this.deps.durable.get(claimTxKey(rec.id));
    ...
    try {
      const hist = await client.getHistory(getHTLCScripthash(redeemScript), scriptHex);
      if (Array.isArray(hist) && hist.some((h) => typeof h?.tx_hash === "string" && h.tx_hash.toLowerCase() === claimTxid)) return false;
      const utxos = await client.getUTXOs(getHTLCScripthash(redeemScript), scriptHex);
    ...
    try {
      await client.broadcastTx(claim.rawTx);
      return true;
    } catch {
      return false;
    }
   ...
   */
  async broadcastRefundWithSentinelGuard(client, rawTx, id) {
    try {
      await client.broadcastTx(rawTx);
    } catch (e) {
      if (isDefinitiveBroadcastRejection(e)) {
        try {
          await this.deps.durable.remove(refundBroadcastKey(id));
        } catch {
        }
      }
      throw e;
  ...
  async recoverUtxoRefundRace() {
    ...
    let history;
    try {
      history = await client.getHistory(getHTLCScripthash(redeemScript), scriptHex);
    } catch {
      return false;
    }
    ...
    for (const item of history) {
      ...
      if (!HEX64.test(txid) || txid === refundTxid) continue;
      let raw;
      try {
        raw = await client.getTx(txid);
      } catch {
      ...
      let s;
      try {
        s = extractSecret(raw, hashLockHex);
      } catch {
        s = null;
      }
      if (!s || s.length !== 32) continue;
      if (bytesToHex(sha256(s)) !== hashLockHex) continue;
      if (this.secret) this.secret.fill(0);
      this.secret = s;
      try {
        await this.deps.durable.remove(refundBroadcastKey(rec.id));
      ...
      try {
        await this.claimWithKnownSecret();
      } catch {
      }
      return true;
    }
   ...
   */
  async finalizeOrResubmitEvmRefund() {
    const rec = this.record;
    if (!chainConfigs[this.myChain]?.isEvm) return false;
    if (!await this.deps.durable.get(refundBroadcastKey(rec.id))) return false;
    const swapId = (rec.myEvmSwapId ?? "").toLowerCase();
    ...
    if (sw.refunded) {
      this.setPhase("refunded");
      this.status("finalizeOrResubmitEvmRefund:finalized");
      await this.persistRecord();
      return true;
    }
    if (sw.claimed) return false;
