...
// R-CPRECIP-001: the recipient pkh + secretHash every test HTLC (htlcRedeem / htlcRedeemRaw) commits — the gate binds
// the counterparty script against these, so the happy tests must pass the matching values.
const RS_RECIP_PKH = new Uint8Array(20).fill(0x11);
const RS_SECRETHASH = new Uint8Array(32).fill(0xa5);
...
describe('assertRevealSafe (initiator secret-reveal gate)', () => {
  ...
  it('HAPPY (height-CLTV): deep+fresh+margin-ok mints an OUTPOINT-BOUND RevealAuthorization', async () => {
    const auth = await assertRevealSafe(utxoClient(CTX), {
      role: 'initiator', theirChain: CHAIN, counterpartyRedeemScript: REDEEM, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH, expectedFundedValueSats: 100000,
  ...
  it('RESPONDER (already-public secret) is NOT margin-blocked even with a near-expiry counterparty leg', async () => {
    ...
    useChain(ctx);
    const auth = await assertRevealSafe(utxoClient(ctx), {
      role: 'responder', theirChain: CHAIN, counterpartyRedeemScript: redeem, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH, expectedFundedValueSats: 100000,
  ...
  it('FAIL-CLOSED (R-UNDERFUND-001): a dust-funded counterparty leg Y (authenticated value < offer amount) aborts — never reveal S against an under-funded leg', async () => {
    ...
    await expect(assertRevealSafe(utxoClient(CTX), {
      role: 'initiator', theirChain: CHAIN, counterpartyRedeemScript: REDEEM, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH,
      recordedOutpoint: outpoint(CTX), counterpartyLocktime: HEIGHT_LOCKTIME,
      expectedFundedValueSats: 100001, // one sat above the authenticated funded value (100000)
    })).rejects.toMatchObject({ name: 'GateFailure', disposition: 'abort' });
  });

  it('FAIL-CLOSED (R-CPRECIP-001): a counterparty HTLC naming a DIFFERENT recipient pkh (attacker key) aborts — we could never claim it', async () => {
    // A REAL, self-consistent HTLC funded on-chain (its P2SH matches, CLTV + value pass) but whose recipient pkh is the
    // ATTACKER's (0x33), not ours (0x11). Before the fix the gate authenticated only the funding-to-script link and
    // MINTED — then we reveal S and the maker sweeps this leg while OUR claim (needs our key) is script-invalid.
    const attackerRedeem = createHTLCRedeemScript({
      secretHash: RS_SECRETHASH, recipientPubkeyHash: new Uint8Array(20).fill(0x33), refundPubkeyHash: new Uint8Array(20).fill(0x22), locktime: HEIGHT_LOCKTIME,
    });
    const actx = buildSynthChain({ anchorHeight: 100000, count: 4, spacing: 600, bits: 0x20010000, fundSpkHex: p2shSpk(attackerRedeem) });
    useChain(actx);
    await expect(assertRevealSafe(utxoClient(actx), {
      role: 'initiator', theirChain: CHAIN, counterpartyRedeemScript: attackerRedeem,
      recordedOutpoint: outpoint(actx), counterpartyLocktime: HEIGHT_LOCKTIME,
      expectedFundedValueSats: 100000, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH, // we expect OUR pkh (0x11)
    })).rejects.toMatchObject({ name: 'GateFailure', disposition: 'abort' });
  });

  it('FAIL-CLOSED (R-CPRECIP-001): a counterparty HTLC committing a DIFFERENT secretHash aborts — the swap secret would not unlock it', async () => {
    const wrongHashRedeem = createHTLCRedeemScript({
      secretHash: new Uint8Array(32).fill(0xbe), recipientPubkeyHash: RS_RECIP_PKH, refundPubkeyHash: new Uint8Array(20).fill(0x22), locktime: HEIGHT_LOCKTIME,
    });
    const bctx = buildSynthChain({ anchorHeight: 100000, count: 4, spacing: 600, bits: 0x20010000, fundSpkHex: p2shSpk(wrongHashRedeem) });
    useChain(bctx);
    await expect(assertRevealSafe(utxoClient(bctx), {
      role: 'initiator', theirChain: CHAIN, counterpartyRedeemScript: wrongHashRedeem,
      recordedOutpoint: outpoint(bctx), counterpartyLocktime: HEIGHT_LOCKTIME,
      expectedFundedValueSats: 100000, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH, // we expect the offer hash (0xa5)
    })).rejects.toMatchObject({ name: 'GateFailure', disposition: 'abort' });
  });
});
...
describe('assertLegBuriedForFunding (responder fund-Y gate)', () => {
  ...
  it('HAPPY: leg-X buried + responder margin ok mints an OUTPOINT-BOUND FundProof', async () => {
    const proof = await assertLegBuriedForFunding(utxoClient(CTX), {
      theirChain: CHAIN, myChain: 'bch2', myChainIsEvm: false, counterpartyRedeemScript: REDEEM, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH, expectedFundedValueSats: 100000,
  ...
  it('EVM-responder margin branch (myChainIsEvm=true uses RESPONDER_LOCK_SEC): a leg too tight for the 12h wall-clock lock throws (abort)', async () => {
    ...
    useChain(ctx);
    await expect(assertLegBuriedForFunding(utxoClient(ctx), {
      theirChain: CHAIN, myChain: 'poly', myChainIsEvm: true, counterpartyRedeemScript: redeem, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH, expectedFundedValueSats: 100000,
      recordedOutpoint: outpoint(ctx), counterpartyLocktime: SYNTH_TIP + 100,
    })).rejects.toMatchObject({ name: 'GateFailure', disposition: 'abort' });
  });
  ...
  it('FAIL-CLOSED (R-UNDERFUND-001): a dust-funded leg X (authenticated value < offer.sendAmount) aborts — the responder never funds leg Y against a dust leg X', async () => {
    ...
    await expect(assertLegBuriedForFunding(utxoClient(CTX), {
      theirChain: CHAIN, myChain: 'bch2', myChainIsEvm: false, counterpartyRedeemScript: REDEEM, expectedRecipientPkh: RS_RECIP_PKH, expectedSecretHash: RS_SECRETHASH,
      recordedOutpoint: outpoint(CTX), counterpartyLocktime: HEIGHT_LOCKTIME,
      expectedFundedValueSats: 100001, // one sat above the authenticated funded value (100000)
    })).rejects.toMatchObject({ name: 'GateFailure', disposition: 'abort' });
  });
