...
export interface ChainClient {
  ...
  getBlockHeaders(start: number, count: number): Promise<{ count: number; hex: string; max: number }>;

  /** R175-SPV: Merkle inclusion proof {block_height, merkle (display hex), pos} for a funding tx at a height. */
  getMerkleProof(txid: string, height: number): Promise<{ block_height: number; merkle: string[]; pos: number }>;
